...
sns.scatterplot(
    data=cars.assign(mpg=np.log(cars.mpg)),
    x="horsepower",
    y="mpg",
...
sns.scatterplot(
    data=cars.assign(mpg=np.log(cars.mpg), horsepower=np.log(cars.horsepower)),
    x="horsepower",
    y="mpg",
    color=sns.cubehelix_palette()[-1],
    alpha=0.5,
...
for i, (ax, origin) in enumerate(zip(axs, cars.origin.unique())):
    gp.fit(
        outputs=["mpg"],
        categorical_dims=["origin"],
        categorical_levels={"origin": origin},
...
for ax, linear in zip(axs, [False, True]):
    ...
    gp.fit(
        outputs=["mpg"],
        continuous_dims=["horsepower", "weight"],
    ...
    cs = ax.contour(
        X.values(), Y.values(), z.σ / z.μ, levels=4, colors="0.2", linestyles="--"
    )
    ax.clabel(cs)
...
gp.fit(
    outputs=["mpg"],
    continuous_dims=["horsepower", "weight", "cylinders"],
    linear_dims=["horsepower", "weight", "cylinders"],
)
...
for ax, cyl in zip(axs, cylinders):
    ...
    cs = ax.contour(
        X.values(), Y.values(), z.σ, levels=4, colors="0.2", linestyles="--"
    )
    ax.clabel(cs)
