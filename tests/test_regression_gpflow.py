...
import pymc3 as pm
import gpflow
from gumbi import DataSet, Standardizer
from gumbi import GP_gpflow as GP
...
def test_gp_build_model_simple(example_gp):
    gp = example_gp.specify_model(continuous_dims=['X', 'Y'])
    gp.build_model()
    assert isinstance(gp.model, gpflow.models.gpr.GPR) or isinstance(gp.model, gpflow.models.sgpr.SGPR)
    assert isinstance(gp.gp_dict['total'], gpflow.models.gpr.GPR) or \
           isinstance(gp.gp_dict['total'], gpflow.models.sgpr.SGPR)
...
def test_gp_build_model(example_gp, fit_inputs, additive):
    ...
    gp.build_model()
    assert isinstance(gp.model, gpflow.models.gpr.GPR) or isinstance(gp.model, gpflow.models.sgpr.SGPR)
    assert isinstance(gp.gp_dict['total'], gpflow.models.gpr.GPR) or isinstance(gp.gp_dict['total'],
