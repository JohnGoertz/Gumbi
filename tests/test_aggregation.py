...
from gumbi import DataSet, Standardizer, WideData
...
def test_stdz():
    ...
    assert np.allclose(
        [s.stdz(p, nat_defaults[p]) for p, v in nat_defaults.items()], 0
    ), "Standardization failed"
    assert np.allclose(
        [s.unstdz(p, s.stdz(p, nat_defaults[p])) for p, v in example_stdzr.items()],
        [v for v in nat_defaults.values()],
    ), "Round-trip standardization failed"
    ...
    assert np.isclose(stdzr.stdz("x", 1 + 0.1**0.5), 1.0)
...
def example_dataset():
    ...
    ds = DataSet.from_tidy(
        df, names_column="Parameter", log_vars=["Y", "c", "b"], logit_vars=["X", "e"]
    )
...
def test_tidy_z(example_dataset):
    ...
    assert np.allclose(
        [
            ds.tidy.z[ds.tidy.z.Parameter == p]["Value"].mean()
            for p in ds.tidy.z.Parameter.unique()
        ],
        0,
    )
...
def test_wide_io(example_dataset):
    ...
    wide_out = ds.wide
    wide_in_wd = WideData(
        wide_out, outputs=ds.outputs, log_vars=["Y", "c", "b"], logit_vars=["X", "e"]
    )
    wide_in_ds = DataSet(
        wide_out, outputs=ds.outputs, log_vars=["Y", "c", "b"], logit_vars=["X", "e"]
    )
